...
class GalleryGrid<T extends SizeType> {
  private _items: T[];
  containerWidth: number;
  gap: number;
  rowHeightRange: RangeType;
  ...
  constructor({
    ...
    if (rowHeightRange) {
      if (rowHeightRange.min < 0 || rowHeightRange.max < 0)
        throw new Error("Row height range should be positive");
      if (rowHeightRange.max < rowHeightRange.min)
        throw new Error("Row height range minimum should be less than maximum");
    }
    this.rowHeightRange = rowHeightRange || defaultHeightRange;
...
class Row<T extends SizeType> {
  ...
  get ratioRange(): RangeType {
    ...
    const minRatio = rowWidth / this._grid.rowHeightRange.max;
    const maxRatio = rowWidth / this._grid.rowHeightRange.min;
    return {
      min: minRatio,
      max: maxRatio,
    };
